...
def corr(values: np.ndarray):
    ...
    The inner sum over i is the unnormalized autocovariance at lag k, so one
    dot product covers each lag. The sum still stops at the first non-positive
    lag, which keeps the cost O(n*l) rather than O(n^2).

    The ``k = n`` term is unreachable and so left out of the loop: with the
    mean subtracted, ``sum_{k>=1} acov(k) = -acov(0)/2 < 0``, so some lag is
    always non-positive and the break always fires by ``n - 1``.
    ...
    variance = deviation.dot(deviation) / (n - 1)

    if np.isclose(np.sqrt(variance), 0.0):
        return np.inf
    corr_len = 0.0
    for k in range(1, n):
        ct = (deviation[: n - k].dot(deviation[k:]) / variance) / (n - k)
        if ct <= 0.0:
            break
